...
class PhotoDeleteService(ServiceWithResult):
    ...
    def process_comments(self, comments):
        for comment in comments:
            send_notification(User.objects.get(username=comment.user).id,
                              'Фотография, к которой Вы оставили комментарий, отправлена на удаление, скоро Ваш комментарий будет удален')
            if comment.comments:
